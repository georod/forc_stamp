...
#polyChGr <- readRDS(paste0(outf4, "polyChGreen.rds"))
...
# # Create plot
# png(file=paste0(outf5, trendLabs[1], "_global1", ".png"),
#       units = "in",
#       width = 6,
#       height = 3.5,
#       res = 300) 
...
#                 polyL[[3]],
#                 dist=-5,   # Did not work with -1 like before. -5 worked this time. Strange.
#                 nQuadSegs = 30,
#                 endCapStyle = "SQUARE",
#                 joinStyle = "MITRE",
#                 mitreLimit = 1,
...
#polyChGr[[3]] <- stamp(polyL2[[3]], polyL2[[4]], dc = 0, direction = FALSE, distance = FALSE)
 ...
 #saveRDS(polyChGr,  paste0(outf4, "polyChGreen.rds")) # 1, & 4 with -1 buff, 2 & 3 with -5. Now comparison 3 did work.
...
# 
# png(file=paste0(outf5, trendLabs[1], "_prop_change_events1", ".png"),
#       units = "in",
#       width = 5,
#       height = 6,
#       res = 300)
...
# PLoting rate of spread (distance over per time (5 years))
# prepard for ggplot

# distL1 <- foreach(i=1:length(polyChGrDistL)) %dopar% {
#                  
#         temp1 <- na.omit(polyChGrDistL[[i]][,c("AREA", "GROUP", "CENDIST"), drop=TRUE])
#         temp1$Period <- periodYrLabs3[i]
#               temp1    
#                  }
# 
# dist1 <- do.call(rbind, distL1)
# dist1$Period <- as.factor(dist1$Period)
...
# Calculate mean
# meanDistPer <- ddply(dist1, "Period", summarise, grp.mean=mean(CENDIST))
# meanDistP
# 
# 
# png(file=paste0(outf5, trendLabs[1],"_distance1", ".png"),
#       units = "in",
#       width = 5,
#       height = 6,
#       res = 300)
#       
# ggplot(dist1, aes(x=CENDIST, colour=Period, fill=Period)) + geom_density(alpha=0.5) + theme_minimal() + geom_vline(data=meanDistPer, aes(xintercept=grp.mean, color=Period), linetype="dashed", size=1) +  labs(x="Centroid distance (m)", y = "Density") + theme(legend.position = 'bottom')
#              
...
# png(file=paste0("~/forc_stpa/", trendLabs[1], "_direction1", ".png"),
#       units = "in",
#       width = 6,
#       height = 3.5,
#       res = 300)
