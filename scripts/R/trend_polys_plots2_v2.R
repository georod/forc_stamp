...
infolder1 <- "~/forc_stamp/output1/change_poly"
...
#sf::st_area(st_transform(st_read(shp1), proj1))/10000
study_area <- as.numeric(sf::st_area(st_read(shp1)))/10000 ## box Sinu=1447817 ha

#study_area <- as.numeric(sf::st_area(st_transform(st_read(shp1), crs=3347)))/10000 ## box ## 3347: Statistics Canada Lambert; 1493031 ha

#studyAreaHa <- 1447817 # based on vector study area
forestAreaHa <- c(1324087) # Based of shp found in Postgres which is based on 30 m raster from 2003.
#proforestAreaHa <- c(220046.5) # protected area (including forest) based on 250 m raster
#nonproforestAreaHa <- c(1324087) # non-protected area (including forest) based on 250 m raster
#proforestAreaHa+nonproforestAreaHa #1544134, which is the area stated in chap. 2

#250 m
# for2003 <- terra::rast("~/forc_stpa/output1/forest_land_cover_2003_250m_v1.tif")
# freq(for2003)
# 
# for2003_v2 <- terra::subst(for2003, 1, NA )
# expanse(for2003_v2, unit="ha")

#30x30 in Canada Atlas Lambert, 3978
# for2003_30m <- terra::vect("~/forc_stpa/output1/gis/forest_lcover/forest_lcover_2003.gpkg")
# sum(expanse(for2003_30m))/10000 #1287026
# sum(expanse(project(for2003_30m, "epsg:3347"), unit='ha')) #1287026
# sum(expanse(project(for2003_30m, "+proj=sinu +lon_0=0 +x_0=0 +y_0=0 +R=6371007.181 +units=m"), unit='ha')) #1287026

# Using object found in POstgres (forest_lcover_2003_pro_v3)

proforestAreaHa <- c(220047) # protected area (including forest) based on 250 m raster
nonproforestAreaHa <- c(1104040) # non-protected area (including forest) based on 250 m raster
...
#sum_area_for_status_greening_EVENT <- c(nonproforestAreaHa, proforestAreaHa)
#sum_area_for_status_browning_EVENT <- c(nonproforestAreaHa, proforestAreaHa)
...
#sum(expanse(vect(pgTabsL[[1]])))/10000

chPoly <- foreach (i=1:length(pgTabsL), .combine = rbind) %do% {
  
  temp1 <- sf::st_transform(pgTabsL[[i]], 3347)
  # In Postgis script non-protected=0, protected=1. Below I recode this to 1 and 2 respectively to aid indexing
  # This way, I can use indices more easily
  ...
  sum_area_nonproFor <- sum(st_area(temp1[which(temp1$status==1),]))/1e4
  sum_area_proFor <-sum(st_area(temp1[which(temp1$status==2),]))/1e4
  ...
  foreach(j=1:length(trendChangeLabs), .combine=rbind) %do% {
    ...
    sum_poly$percent <- ifelse(k==1,(sum_poly$area/sum_area_nonproFor)*100, (sum_poly$area/sum_area_proFor)*100) # event area
    #sum_poly$avg_area <- sum_poly$area/sum_poly$number  # number of polygons no longer makes sense when dividing by protected status because a given patch can span both protected and non-protected forest polygons
...
png(file=paste0(outf1, "green_poly_change", "_per_area_patches_pro_global1", ".png"),
    units = "in",
    width = 6,
    height = 3.5,
    res = 300)
...
ggplot(data=chPoly[chPoly$trend=="Greening",], aes(x=periodYrLabs3, y=percent, fill=level2)) +
  ...
  scale_fill_manual(values=c("#f28304", "#cc0c24", "#305a02", "#7ee61c", "#181e1e")) +
  facet_grid(~ factor(status, c("Protected", "Non-protected"))) + theme_bw() + theme(legend.position="bottom") + labs(fill = "Change type:")

dev.off()


png(file=paste0(outf1, "brown_poly_change", "_per_area_patches_pro_global1", ".png"),
    units = "in",
    width = 6,
    height = 3.5,
    res = 300)
...
#-------------------------------------------
# Plots with not protected status
p1Poly <- sqldf::sqldf("SELECT 1 as period, trend, change,  periodYrLabs3, sum(area) area, sum(number) as number FROM chPoly
WHERE level2 IN ('cont','disa','stbl') and change='ch1'
GROUP BY trend, change,  periodYrLabs3")

p2Poly <- sqldf::sqldf("SELECT 2 as period, trend, change,  periodYrLabs3, sum(area) area, sum(number) as number FROM chPoly
WHERE level2 IN ('expn','genr','stbl') and change='ch1'
GROUP BY trend, change,  periodYrLabs3")

p3Poly <- sqldf::sqldf("SELECT 3 as period, trend, change,  periodYrLabs3, sum(area) area, sum(number) as number FROM chPoly
WHERE level2 IN ('cont','disa','stbl') and change='ch3'
GROUP BY trend, change,  periodYrLabs3")

p4Poly <- sqldf::sqldf("SELECT 4 as period, trend, change,  periodYrLabs3, sum(area) area, sum(number) as number FROM chPoly
WHERE level2 IN ('expn','genr','stbl') and change='ch3'
GROUP BY trend, change,  periodYrLabs3")
...
#-------------------------------------------------------------------
# Get Greening and browning area to check with areas estimated above -- Code below not run
#-------------------------------------------------------------------

# I checked several areas and they do match. So, it is ok to derive green and browning with
