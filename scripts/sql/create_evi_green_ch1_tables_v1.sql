...
DROP TABLE evi_green_ch1_inter_v1;

CREATE TABLE evi_green_ch1_inter_v1 AS
...
ALTER TABLE evi_green_ch1_inter_v1 ADD CONSTRAINT evi_green_ch1_inter_v1 PRIMARY KEY (gid)

COMMENT ON TABLE evi_green_ch1_inter_v1 IS 'EVI greening period 1 vs. period 2 change, stable polygons (intersection). [2024-12-14]'

-- Index: evi_poly_type1_16d2_p1_geom_idx

-- DROP INDEX IF EXISTS public.evi_poly_type1_16d2_p1_geom_idx;

CREATE INDEX IF NOT EXISTS evi_green_ch1_inter_v1_geom_idx
    ON public.evi_green_ch1_inter_v1 USING gist
    (geom)
...
JOIN
(
SELECT patches, array_agg(p2_patches) AS array_p2_patches, st_union(geom) AS geom FROM evi_green_ch1_inter_v1
...
JOIN
(
SELECT p2_patches, array_agg(patches) AS array_p1_patches, st_union(geom) AS geom FROM evi_green_ch1_inter_v1
...
CREATE VIEW evi_green_ch1_level2_v1 AS
SELECT row_number() over () as rowid, gid, patches, period, level2, geom::geometry(Multipolygon, 100000) AS geom
FROM
(
SELECT gid, patches, period, level2, geom FROM evi_green_ch1_inter_v1
UNION ALL
SELECT gid, patches, period, level2, geom FROM evi_green_ch1_cont_v1 
UNION ALL
SELECT gid, patches, period, level2, geom FROM evi_green_ch1_disa_v1 
UNION ALL
SELECT gid, patches, period, level2, geom FROM evi_green_ch1_genr_v1 
UNION ALL
SELECT gid, patches, period, level2, geom FROM evi_green_ch1_expn_v1 
) t1
-- 10355
