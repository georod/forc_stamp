...
BEGIN
    FOR i IN 1..3 LOOP
        new_tbl_name := 'evi_brown_ch' || i ||'_level2_int_v1';  -- New table to be created (e.g., new_table_1, new_table_2)
        join_tbl_name := 'evi_brown_ch' || i||'_level2_v1';     -- Table to join (e.g., table_1, table_2)
		index_name := 'evi_brown_ch' || i ||'_level2_int_v2_geom_idx'; -- New index name
		table_comment := 'EVI browning trend polygon changes (level2) for ' || year_labels[i]||' with protected area status. [2025-01-30]';
...
BEGIN
    FOR i IN 1..3 LOOP
        new_tbl_name := 'evi_brown_ch' || i ||'_level2_diff_v1';  -- New table to be created (e.g., new_table_1, new_table_2)
        join_tbl_name := 'evi_brown_ch' || i||'_level2_v1';     -- Table to join (e.g., table_1, table_2) 
		index_name := 'evi_brown_ch' || i ||'_level2_diff_v1_geom_idx'; -- New index name
		table_comment := 'EVI browning trend polygon changes (level2) for ' || year_labels[i]||' with protected area status. These polygons are the difference of the intersection with protected areas. [2025-01-30]';
		

-- Step 1: Create the new table with all data except the primary key	
        EXECUTE format('  
CREATE TABLE %I AS
SELECT t1.gid AS gid_ch, t1.patches, t1.period, t1.level2, t2.gid AS gid_pro, t2.gid0 AS gid0_pro, 0 AS status, St_Multi((ST_DUMP(st_difference(t1.geom, t2.geom))).geom)::geometry(MultiPolygon,100000) AS geom
FROM
%I t1
LEFT JOIN
(SELECT gid, __gid as gid0, 1 AS status, st_transform(geom, 100000) geom FROM cpcad_dec2020_clipped2) t2
...
BEGIN
    FOR i IN 1..3 LOOP
        new_tbl_name := 'evi_brown_ch' || i ||'_level2_not_int_v1';  -- New table to be created (e.g., new_table_1, new_table_2)
        join_tbl_name := 'evi_brown_ch' || i||'_level2_v1';     -- Table to join (e.g., table_1, table_2)
		index_name := 'evi_brown_ch' || i ||'_level2_not_int_v1_geom_idx'; -- New index name
		table_comment := 'EVI browning trend polygon changes (level2) for ' || year_labels[i]||' that do not intersect with protected areas. [2025-01-30]';
...
SELECT 'INT-'||gid::text as gid2, * FROM evi_brown_ch3_level2_int_v1
UNION
SELECT 'DIFF-'||gid::text as gid2, * FROM evi_brown_ch3_level2_diff_v1
UNION
SELECT 'NINT-'||gid::text as gid2, * FROM evi_brown_ch3_level2_not_int_v1
;
...
SELECT 'INT-'||gid::text as gid2, * FROM evi_brown_ch2_level2_int_v1
UNION
SELECT 'DIFF-'||gid::text as gid2, * FROM evi_brown_ch2_level2_diff_v1
UNION
SELECT 'NINT-'||gid::text as gid2, * FROM evi_brown_ch2_level2_not_int_v1
;
...
SELECT 'INT-'||gid::text as gid2, * FROM evi_brown_ch1_level2_int_v1
UNION
SELECT 'DIFF-'||gid::text as gid2, * FROM evi_brown_ch1_level2_diff_v1
UNION
SELECT 'NINT-'||gid::text as gid2, * FROM evi_brown_ch1_level2_not_int_v1
;
